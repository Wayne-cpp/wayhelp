...
5. 只用 calibration 分片选择 `knowledge_min_score`: 在 COSINE [-1, 1] 范围搜索阈值,先满足负例误召回率 ≤ 10%,再最大化正例 Recall@5,指标相同时取较高阈值。校准结果未达 Recall@5 ≥ 90% 时同样判失败。冻结阈值后才运行 test 分片,不得根据 test 结果反复调阈值再宣称独立验收。
