...
**R3(2026-09-20,评审 Important #2 用户裁定):refund_policy gather 改部分失败容忍。** 裁定「按 spec 改 + 补钉」——spec §6.4 原文即「仅扩写支路的临时异常可丢弃该支路并记录原因;base_query 检索异常/超时或维护态才走 unavailable」,实现却做成 all-or-nothing gather(任一支路异常整轮 kb_unavailable)。按 spec 对齐:`return_exceptions=True` + base 异常/总超时仍整轮不可用,扩写支路异常丢弃并按既有 join 约定记 `expansion_branch_dropped:<原因>` note,全支路丢弃退化为 base-only;补 §13「部分扩写异常」两条故障钉(单支路/全支路,red 先行)。
