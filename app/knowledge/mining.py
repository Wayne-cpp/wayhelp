...
不抽取客服「无法回答/查不到/请用户提供更多信息核实」这类失败应答——那不是知识。
输出严格的 JSON 对象,不要输出任何其他内容。
JSON 结构(键名必须逐字遵守):一个对象,仅一个键 "conversations",值为数组;
数组每项为 {"conversation_id": <会话 id 整数>, "items": [{"question": "...", "answer": "..."}]},
没有可抽取内容时 items 为空数组。"""  # 尾两段必填:json_mode(DeepSeek)要求提示词含 json 字样;schema 不进 API 参数,键名只能写进提示词(实测模型会自造 session_id/items 顶层键)
